...
namespace CryptoCAD.Core.DES
{
    public class DESService
    {
        ...
        public static uint BITNUM(byte[] a, int b, int c)
        {
            return (uint)(((a[b / 8] >> (7 - (b % 8))) & 0x01) << (c));
        }

		public static byte BITNUMINTR(uint a, int b, int c)
        {
            return (byte)((((a) >> (31 - (b))) & 0x00000001) << (c));
        }

		public static uint BITNUMINTL(uint a, int b, int c)
        {
            return ((((a) << (b)) & 0x80000000) >> (c));
        }

		public static uint SBOXBIT(byte a)
        {
            return (uint)(((a) & 0x20) | (((a) & 0x1f) >> 1) | (((a) & 0x01) << 4));
        }

		public static byte[] sbox1 = {
	...
	2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11
		...
		public static void IP(uint[] state, byte[] input)
		{
			int i, j;
			...
			for (i = 31, j = 32; i >= 0; i--, j++)
            {
				state[0] |= BITNUM(input, ip[j], i);
			}

			for (i = 31, j = 0; i >= 0; i--, j++)
			{
				state[1] |= BITNUM(input, ip[j], i);
			}
		...
		public static void InvIP(uint[] state, byte[] input)
		{
